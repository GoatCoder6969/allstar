...
db_curse = db["curse_players"]  # Collection for Curse players 
db_duke = db["duke_players"]  # Collection for Duke players

# Track active database
current_db = db_duke  # Default database
current_db_name = "Duke"  # Track the name for response messages
...
SESSION_STRING = "BQA4ntkAZz2O_vwVpxHlVzq5MYVA-oJ91mXHF00_IovJaq8OIhs4g0B0_vvjzFq5H9_mA6nPLvC9D2ql6CyrW2FmDy-unsM8EL1ddwYq-YpAcxb-vyQCKYdrGQzDdclrv77l8Tgg1EeTFj8J70quwL2vUB_asntunHLpzTCoqHy8hPZVA8QFCcvU1I2CdIaePqTuL9vfx6EQMjwUfYcvogR1EQQ7RtREqov_nZMracXCKyp_e9zoqRhdjq-TUhrTQowfPQUUtbnjUzRb7fD8t0NrUQ-yIEmHcFa2EcH1VxC89eh4hBqPwAku2DESP55oHDr2sc8I4b-jZP0PHxspzLpiSNtpfAAAAABohTyKAA"
bot = Client(
    "pro",
    api_id=int(API_ID),
    api_hash=API_HASH,
...
# Admin User IDs
ADMIN_USER_IDS = [1745451559, 1710597756, 7522153272, 7946198415, 7742832624, 7859049019, 7828242164, 7957490622, 7323291282, 6523029979, 7921822971, 6678433785, 1259702343, 7254828091, 7546387669, 7546387669, 6895497681, 1753562250]
# User IDs permitted to trigger the collect function
COLLECTOR_USER_IDS = [7583266644]
...
@bot.on_message(filters.command("switchdb") & filters.chat(TARGET_GROUP_IDS + [MAIN_GROUP_ID]) & filters.user([7508462500, 1710597756, 6895497681, 7435756663, 7323291282, 6523029979, 6055447708, 7546387669, 6895497681, 1753562250, 6055447708]))
async def switch_database(_, message: Message):
    ...
    if new_db_name == "curse":
        current_db = db_curse
        current_db_name = "Curse"
    elif new_db_name == "duke":
        current_db = db_duke
        current_db_name = "Duke"
    else:
        await message.reply("⚠ Invalid database! Use: `/switchdb curse` or `/switchdb duke`")
        return
...
async def import_json_to_mongodb(_, message: Message):
    ...
    try:
        ...
        if db_name not in ["curse", "duke"]:
            await message.reply("⚠ Invalid database! Use either `curse` or `duke`")
            return

        # Select target database
        target_db = db_curse if db_name == "curse" else db_duke
        db_display_name = "Curse" if db_name == "curse" else "Duke"
    ...
    try:
        ...
        if len(message.command) < 3:
            await message.reply(
                ...
                "**Example**: `/addplayer curse John Cena`\n"
...
async def database_info(_, message: Message):
    ...
    try:
        ...
        curse_count = await db_curse.count_documents({})
        duke_count = await db_duke.count_documents({})
        
        # Get sample players from each database
        curse_players = []
        duke_players = []
        
        async for doc in db_curse.find().limit(5):
            curse_players.append(doc["name"])
        
        async for doc in db_duke.find().limit(5):
            duke_players.append(doc["name"])
        
        # Create info message
        info_text = "📊 **Database Information**\n\n"
        
        # Curse DB info
        info_text += "**Curse Database**:\n"
        info_text += f"• Total Players: `{curse_count}`\n"
        if curse_players:
            info_text += "• Sample Players: " + ", ".join([f"`{p}`" for p in curse_players]) + "\n"
        
        info_text += "\n**Duke Database**:\n"
        info_text += f"• Total Players: `{duke_count}`\n"
        if duke_players:
            info_text += "• Sample Players: " + ", ".join([f"`{p}`" for p in duke_players]) + "\n"
